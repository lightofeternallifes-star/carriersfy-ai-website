# CLIENT REGISTRY — Carriersfy AI

> Master index of all clients. Single source of truth for client status and relationships. Last updated: 2026-06-25

---

## Active Clients

### Brazil Signs
| Field | Value |
|---|---|
| **Industry** | Signage / Print / Visual Communication |
| **Geography** | Brazil |
| **Stage** | Active — build status requires assessment |
| **Primary Contact** | Unknown — requires discovery meeting |
| **Active Projects** | [Brazil Signs Project](../PROJECTS/Brazil_Signs/README.md) |
| **Client Folder** | [/CLIENTS/Brazil_Signs/](../CLIENTS/Brazil_Signs/Business_Profile.md) |
| **LTV Tier** | Unknown — TBD post-assessment |
| **Health** | [UNKNOWN — NEEDS ASSESSMENT] |
| **Last Touchpoint** | Unknown |
| **Assigned AI Employees** | Unknown — requires discovery |
| **Action Required** | Schedule discovery/status meeting by 2026-07-05 |

---

### Marine Consolidated Electronics
| Field | Value |
|---|---|
| **Industry** | Marine Electronics / Marine Industry |
| **Geography** | Unknown (likely U.S.) |
| **Stage** | Active — build status requires assessment |
| **Primary Contact** | Unknown — requires discovery meeting |
| **Active Projects** | [Marine Project](../PROJECTS/Marine_Consolidated_Electronics/README.md) |
| **Client Folder** | [/CLIENTS/Marine/](../CLIENTS/Marine/Business_Profile.md) |
| **LTV Tier** | Unknown — TBD post-assessment |
| **Health** | [UNKNOWN — NEEDS ASSESSMENT] |
| **Last Touchpoint** | Unknown |
| **Assigned AI Employees** | Unknown — requires discovery |
| **Action Required** | Schedule discovery/status meeting by 2026-07-05 |

---

### Light of Life
| Field | Value |
|---|---|
| **Industry** | Faith Community / Nonprofit / Religious Organization |
| **Geography** | Unknown |
| **Stage** | Active — build status requires assessment |
| **Primary Contact** | Unknown — requires discovery meeting |
| **Active Projects** | [Light of Life Project](../PROJECTS/Light_of_Life/README.md) |
| **Client Folder** | No dedicated client folder yet — create when details known |
| **LTV Tier** | Unknown — TBD post-assessment |
| **Health** | [UNKNOWN — NEEDS ASSESSMENT] |
| **Last Touchpoint** | Unknown |
| **Assigned AI Employees** | Unknown |
| **Action Required** | Schedule discovery meeting by 2026-07-10 |

---

## Client Pipeline (Not Yet Active)

| Prospect | Industry | Stage | Source | Owner | Target Close |
|---|---|---|---|---|---|
| TBD — Iron Prime outreach | Various | — | Outbound AI | Iron Prime | Q3 2026 |

> Full pipeline tracking: [Future Clients Pipeline](../CLIENTS/Future_Clients/Pipeline.md)

---

## Churned / Inactive Clients

None documented. If a client becomes inactive, move their entry here with:
- Churn date
- Reason (if known)
- Outstanding obligations
- Re-engagement opportunity (Y/N)

---

## LTV Tier Definitions

| Tier | Monthly Value | Annual LTV | Profile |
|---|---|---|---|
| Tier 1 — Enterprise | $3,000+/mo | $36,000+ | Multiple AI employees, complex integrations |
| Tier 2 — Growth | $1,000–$2,999/mo | $12,000–$36,000 | 1–2 AI employees, standard integrations |
| Tier 3 — Starter | $500–$999/mo | $6,000–$12,000 | 1 AI employee, minimal integration |
| Tier 0 — Pilot | Under $500/mo | Under $6,000 | Trial / proof-of-concept phase |

---

## Client Onboarding Process

When a new client is signed:
1. Create a folder under `/OMEGA/CLIENTS/[ClientName]/`
2. Fill in all 9 standard client files (see template in [Future Clients](../CLIENTS/Future_Clients/Onboarding_Template.md))
3. Create a project entry under `/OMEGA/PROJECTS/[ClientName]/`
4. Add an entry to this registry
5. Assign AI employees as scoped
6. Create initial tasks in [TASK_ENGINE](TASK_ENGINE.md)
7. Schedule kickoff meeting via Sofia

---

**Related:** [MASTER_CONTEXT](MASTER_CONTEXT.md) | [PROJECT_REGISTRY](PROJECT_REGISTRY.md) | [KNOWLEDGE_GRAPH](KNOWLEDGE_GRAPH.md) | [Future Clients ICP](../CLIENTS/Future_Clients/ICP.md)
